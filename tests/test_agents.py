from backend.schemas import BasketItem, OrderRequest, OrderResponse
from backend.services.order_service import OrderService
from backend.services.product_service import ProductService


async def test_order_service_returns_agent_decisions(order_service: OrderService) -> None:
...
async def test_order_created_with_approved_fraud_status(order_service: OrderService) -> None:
...
async def test_order_review_required_high_risk(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="new-user",
...
async def test_order_rejected_out_of_stock(
    order_service: OrderService, out_of_stock_request: OrderRequest
) -> None:
    result = await order_service.create_order(out_of_stock_request)
    assert result.order_status == "rejected_out_of_stock"
    assert result.selected_warehouse is None
    assert len(result.bids) == 0


async def test_multiple_items_order(order_service: OrderService, multiple_item_request: OrderRequest) -> None:
    result = await order_service.create_order(multiple_item_request)
    assert result.order_total == round(59.99 * 2 + 199.99 * 1, 2)
    assert result.order_status in {"created", "review_required"}
    assert len(result.bids) == 3


async def test_inventory_reservation_for_approved_order(order_service: OrderService, valid_order_request: OrderRequest) -> None:
    initial_stock = order_service.product_service.products["p-1001"].quantity
    result = await order_service.create_order(valid_order_request)
    if result.order_status == "created":
        assert order_service.product_service.products["p-1001"].quantity == initial_stock - 1


async def test_warehouse_bids_structure(order_service: OrderService, valid_order_request: OrderRequest) -> None:
    result = await order_service.create_order(valid_order_request)
...
async def test_course_trace_present(order_service: OrderService, valid_order_request: OrderRequest) -> None:
    result = await order_service.create_order(valid_order_request)
    assert len(result.course_trace) == 3
...
async def test_model_evaluations_present(order_service: OrderService, valid_order_request: OrderRequest) -> None:
    result = await order_service.create_order(valid_order_request)
    assert len(result.model_evaluations) == 2
...
async def test_restock_recommendation_logic(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
        items=[BasketItem(product_id="p-1001", quantity=1)],
        shipping_distance=10,
