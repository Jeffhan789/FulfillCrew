...
from backend.schemas import BasketItem, OrderRequest


async def test_product_service_loads_products() -> None:
...
async def test_product_service_get_product_map() -> None:
    service = ProductService()
    product_map = await service.get_product_map()
    assert isinstance(product_map, dict)
    assert "p-1001" in product_map


async def test_order_service_init(product_service: ProductService) -> None:
...
async def test_order_total_calculation(order_service: OrderService) -> None:
...
async def test_order_with_unknown_product_rejected(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
...
async def test_order_uuid_generated(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
...
async def test_demand_prediction_positive(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
        items=[BasketItem(product_id="p-1001", quantity=1)],
        shipping_distance=10,
...
async def test_inventory_reservation_reduces_stock(order_service: OrderService) -> None:
...
async def test_model_evaluations_fields(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
        items=[BasketItem(product_id="p-1001", quantity=1)],
        shipping_distance=10,
...
async def test_decision_log_contains_agent_names(order_service: OrderService) -> None:
    request = OrderRequest(
        user_id="test-user",
        items=[BasketItem(product_id="p-1001", quantity=1)],
        shipping_distance=10,
