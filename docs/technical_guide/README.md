...
> 本文档是 FulfillCrew（智仓通）项目的技术原理教学文档集，专为项目作者设计，帮助理解每个技术选型背后的原理、代码实现细节，以及如何在面试中表达。
...
| 02 | [前端技术深入](02_frontend_deep_dive.md) | React 18 并发特性、Vite 构建原理、useOrderSocket Hook、Recharts 可视化 | 前端面试准备 |
| 03 | [后端技术深入](03_backend_deep_dive.md) | FastAPI lifespan、SQLAlchemy 2.0 async、Repository 模式、Pydantic 验证、异步编程 | 后端面试准备 |
| 04 | [多智能体系统](04_multi_agent_system.md) | Contract Net Protocol、6 个 Agent 详解、竞价策略公式、OrderService 编排 | 分布式系统/AI 面试 |
| 05 | [机器学习模型](05_ml_models_deep_dive.md) | PyTorch MLP 架构、XGBoost + SHAP、TF-IDF + LR、合成数据生成、模型评估指标 | 机器学习面试 |
| 06 | [基础设施与可观测性](06_infrastructure_observability.md) | Redis/InMemory 事件总线、structlog 结构化日志、Prometheus 指标、健康检查、WebSocket | 运维/SRE 面试 |
| 07 | [Docker 与部署](07_docker_deployment.md) | 多阶段构建、Nginx 反向代理、Docker Compose 编排、环境变量管理、CI/CD 概念 | DevOps 面试 |
| 08 | [面试问答](08_interview_qa.md) | 25+ 高频面试题 + 参考答案 + 追问方向 + 常见陷阱 | 面试冲刺阶段 |
...
3. 最后读 [08 面试问答](08_interview_qa.md)，检验理解程度

### 路径二：面试导向速查
1. 直接跳到 [08 面试问答](08_interview_qa.md)
2. 遇到不会的题，回到对应章节深入理解
