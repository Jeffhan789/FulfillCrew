# 08 架构复盘问答 —— FulfillCrew 高频架构复盘题与参考答案
...
**参考答案**：
> "原始课程作业是一个简单的 React 购物车，功能停在商品展示和结账。我意识到大多数学生的项目都止步于此，而架构复盘中需要展示的是**工程化思维**和**系统设计能力**。于是我将三门课程的知识点串联：云计算（前端+后端+部署）、多智能体（协调协议）、神经网络（需求预测+欺诈检测），形成一个从下单到履约的完整闭环。这样既展示了课程知识，又体现了将零散功能整合为系统的工程能力。"
