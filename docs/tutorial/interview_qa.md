...
> 按模块分类的深度面试问答，每个问题包含：问题解析 → 标准回答 → 扩展讨论 → 代码示例
...
| 技术 | 选型理由 | 替代方案及为何不选 |
|------|----------|-------------------|
| React 18 + TypeScript | 类型安全、组件化、生态成熟 | Vue/Svelte 也可以，但 React 岗位更多 |
...
> "最关键的点是**课程映射**。COMP310 的课程内容是自主 Agent 协商，如果代码里只是一个大函数，就无法在面试中展示课程知识的应用。"
...
> **Result**：系统可以正确处理正常订单、高风险订单和缺货订单，每个 Agent 的决策都有独立日志。面试时我可以打开浏览器现场演示完整流程。"
...
> 祝面试顺利！记住：回答问题时，先给**结论**，再讲**理由**，最后补**例子/代码**。🎯
