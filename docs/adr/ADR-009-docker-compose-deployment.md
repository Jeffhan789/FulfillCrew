...
| 方案 | 优点 | 缺点 | 结论 |
|------|------|------|------|
| **Docker Compose** | 简单；学习曲线低；适合开发和小型部署；架构复盘标准答案 | 单主机；无自动扩缩容；不是生产编排工具 | ✅ 选中（课程项目） |
...
| 云厂商 PaaS | 托管；自动运维 | 锁定厂商；架构复盘中无法展示底层理解 | ❌ 无法展示技能 |
...
| 风险 | 缓解措施 |
|------|----------|
| Docker Compose 不适合生产级多主机部署 | 架构复盘中明确说明"这是课程项目的容器化展示，生产环境需 K8s 或云托管" |
