...
| 方案 | 优点 | 缺点 | 结论 |
|------|------|------|------|
| **PyTorch MLP** | 完全控制网络结构；接口可扩展；与课程 ELEC320 直接对应 | 需要更多数据才能训练；CPU 推理比 sklearn 慢 | ✅ 选中（课程映射） |
| sklearn MLPRegressor | 简单；无需手动写前向传播 | 网络结构和训练流程控制较少 | ❌ 当前接口优先 PyTorch |
| XGBoost / LightGBM | 表格数据通常表现更好；训练快 | 不是神经网络，与课程 MLP 主题不完全匹配 | ❌ 课程映射不足 |
| ARIMA / 时间序列 | 传统方法；可解释 | 不体现神经网络；对电商需求场景不如 ML 通用 | ❌ 非神经网络 |
| 规则引擎（v1.0） | 零依赖；立即可运行 | 无法从历史样本学习 | ✅ 仅作为降级路径 |

### 欺诈检测模型
| 方案 | 优点 | 缺点 | 结论 |
|------|------|------|------|
| **XGBoost + SHAP** | 适合表格特征；SHAP 提供局部解释 | 不是神经网络；模型文件增加运行依赖 | ✅ 选中 |
| PyTorch 二分类 MLP | 纯神经网络；与课程一致 | 需要更多数据调参；不如 XGBoost 在表格数据上稳定 | ❌ 效果不如 XGBoost |
| Logistic Regression | 简单；可解释；sigmoid 与课程一致 | 线性边界可能无法表达复杂特征交互 | ❌ 当前场景优先 XGBoost |
...
1. 训练数据不足时的 fallback
2. 设计复核中解释 sigmoid/logistic 回归原理的教学工具
...
| 风险 | 缓解措施 |
|------|----------|
| 模型训练数据不足，可能过拟合 | 使用小网络与降级路径；公开模型卡前不声明工业级精度 |
...
| 商品分类准确率不如更重的语言模型 | 将 TF-IDF+LR 明确标注为轻量基线，并通过模型卡记录后续比较 |
