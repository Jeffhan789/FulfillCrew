...
> **Architecture Decision Records (ADR)** — 记录 FulfillCrew（智仓通）项目中的关键架构决策，帮助理解技术选型背后的权衡，并为架构复盘准备提供结构化知识。
...
2. **准备架构复盘**：每个 ADR 包含"架构复盘要点"部分，整理了高频问题和回答思路
3. **扩展项目**：在 ADR 的"权衡与风险"中预留了扩展路径（如升级到 K8s、Redis Streams、BERT 等）

### 作为评审者
可以通过 ADR 快速评估候选人的架构思维：
- 为什么选 A 而不是 B？（技术权衡能力）
- 如果数据量扩大 100 倍，这个架构怎么改？（扩展性思考）
- 这个决策的风险是什么？怎么缓解？（风险意识）

### 作为学习者
1. 按编号顺序阅读，从后端框架到部署，形成完整的架构视图
2. 重点关注"技术细节"和"架构复盘要点"部分
