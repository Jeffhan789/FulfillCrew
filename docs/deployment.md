...
- [ ] Create `.env` file from `.env.example` (never commit secrets to Git)
- [ ] Add TLS termination (Let's Encrypt, Cloudflare, or cloud load balancer)
- [ ] Configure `CORS_ORIGINS` for your production domain
- [ ] Add a PostgreSQL or SQLite service to `docker-compose.yml`
- [ ] Replace in-memory product storage with SQLAlchemy + database
- [ ] Add database migrations (Alembic or similar)
- [ ] Add CI/CD pipeline (GitHub Actions: test -> build -> push -> deploy)
- [ ] Add structured logging and log aggregation
- [ ] Add rate limiting on the order creation endpoint
- [ ] Set up container monitoring (Prometheus + Grafana or cloud-native)
- [ ] Add automated backups for persistent data volumes

## Docker Health Checks

Both containers declare native Docker health checks:
...
| 文件 | 用途 |
|------|------|
| `docker-compose.yml` | 生产环境 -- 多阶段优化构建、健康检查、自动重启 |
...
- [ ] 从 `.env.example` 创建 `.env` 文件（切勿将机密提交到 Git）
- [ ] 添加 TLS 终止（Let's Encrypt、Cloudflare 或云负载均衡器）
- [ ] 为生产域名配置 `CORS_ORIGINS`
- [ ] 向 `docker-compose.yml` 添加 PostgreSQL 或 SQLite 服务
- [ ] 将内存商品存储替换为 SQLAlchemy + 数据库
- [ ] 添加数据库迁移（Alembic 或类似工具）
- [ ] 添加 CI/CD 流水线（GitHub Actions：测试 -> 构建 -> 推送 -> 部署）
- [ ] 添加结构化日志和日志聚合
- [ ] 在订单创建端点添加速率限制
- [ ] 设置容器监控（Prometheus + Grafana 或云原生方案）
- [ ] 为持久数据卷设置自动备份

## Docker 健康检查

两个容器均声明了原生 Docker 健康检查：
